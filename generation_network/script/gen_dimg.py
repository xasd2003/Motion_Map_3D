...
if __name__ == '__main__':
	...
	if new_run_flag:
		...
		for root, dirs, files in os.walk(image_dir):
			if root.split('.')[-1][:3] == 'avi':
				...
				if 'test' in split_file and '_crop_4_mr_0' not in name: continue
				label = get_label(video_name)
				max_id = len(files)
				#print name + ':' + str(max_id)
	...
	input_dir = './data/dimg_input_ucf/' + split_file.split('/')[-1].split('.')[0] + os.sep
	output_dir = '/home/THOUMOS2014/sunyuchao/' + split_file.split('/')[-1].split('.')[0] + os.sep
	...
	error_file = open('./config/error_file', 'w')
	...
	for name in data_info:
		...
		if not os.path.exists(im_dir + os.sep + '%06d' % (2) + '.jpg'):
			continue
	...
	for step in range(max_iter - 1):
		print 'step : %d / %d' % (step + 1, max_iter - 1)
		...
		for name in data_info:
			...
			if step < max_id - 1:
				...
				out_dir = output_dir + os.sep + name + '_L_%d' % (L) + os.sep
				inst_dir = input_dir + os.sep + name + '_L_%d' % (L) + os.sep
				if not os.path.exists(inst_dir + os.sep + '%06d' % (2) + '.jpg'):
					error_file.write('%s\n' % '@@@' + name)
					continue
				list_file.write('%s %d %d\n' % (inst_dir, 1, label))
				output_file.write('%s\n' % (out_dir + '%06d' % (1)))
		...
		for name in data_info:
			...
			if step < max_id - 1:
				...
				inst_dir = input_dir + os.sep + name + '_L_%d' % (L) + os.sep
				out_dir = output_dir + os.sep + name + '_L_%d' % (L) + os.sep
				if not os.path.exists(out_dir + os.sep + '%06d' % (1) + '.conv2a'):
					error_file.write('%s\n' % '###' + name)
					continue
				os.system('rm %s/* -f' % inst_dir)
